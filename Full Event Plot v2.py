...
def trackgraph(data, PlotLocation, size):
    ...
    fig.savefig(PlotLocation,
                dpi=100)  # Save to the given location, at 100dpi. This gives us a 500x500 pixel image with the 5x5 inch size
    plt.close()
...
def Single_Track_Plot(df, plotrange, num_bins, Plot_Location):
    ...
    trackgraph(plotdata, Plot_Location, [540, 540])
...
if __name__ == "__main__":
    ...
    for evtnum in range(evtnum_start, evtnum_end + 1):
        ...
        for plane in planes:
            ...
            red_plane_df = pd.DataFrame(evtlist,
            ...
            for track in tracks:
                ...
                plotdf = trackdf.loc[
                    (trackdf['Plane Corrected'] >= plotrange[0]) & (trackdf['Plane Corrected'] < plotrange[1])]
            ...
            loc = saveloc + str(particle) + "_event_num_" + str(evtnum) + "_Plane_" + str(plane) + "_" + str(
                max(tracks)) + "_Tracks"
