...
"""Correction for plane 1. Flips points along the vertical centre to match the directions of planes 0 and 2."""
def Plane_1_Corr(df):
    df['PlaneCorrected'] = np.nan
    tpc_ind = df.columns.get_loc("TPC")
    wirenum_ind = df.columns.get_loc("Wirenum")
...
#setting up split between test and train data - old no longer used
...
for evtnum in range(evtnum_start, evtnum_end + 1):
    ...
    for plane in planes:
        ...
        corr_plane_df = pd.DataFrame(evtlist, columns = ['Integral', 'PDG', 'PeakTime', 'Plane', 'TPC', 'TrackID', 'Wirenum', 'evtNum', 'PlaneCorrected'])
        

        #find the max and min times that PeakTime holds for data that will be plotted.
        img_df = corr_plane_df.loc[(corr_plane_df['PlaneCorrected'] < (start + evtlen)) & (corr_plane_df['PlaneCorrected'] >= start)]

        
        #bump the colour of the tracks in the image so they look different from the background colour
        #bump = max(img_df['Integral'])*0.05 
        bump = 0
        
        maxtime = max(img_df['PeakTime'])+0.1 #bin data returns Nan if unbinned time is exactly equal to max or min time. Ensure it is always above the max.
        mintime = min(img_df['PeakTime'])-0.1 #bin data returns Nan if unbinned time is exactly equal to max or min time. Ensure it is always below the min.
        #maxtime = 4500
        #mintime = 0
        ...
        plot_df = pd.DataFrame({'evtNum':img_df.loc[:,'evtNum'].values,
                               ...
                               'PlaneCorrected': BinData(img_df, 'PlaneCorrected', planenumbins, maxplane, minplane)})
